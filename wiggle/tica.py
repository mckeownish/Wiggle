...
def tica_from_feature(feature_type, universe):
    
    '''
    Extract a feature type from an MDAnalysis Universe
    Perform tICA returning the projection
    
    input
    feature_type: ramachandran, distance matrix or coordinates
    universe: an mdanaylsis universe
    
    output
    tica_projection for chosen feature type
    '''
    
    F = Featurizer(feature_type)
    feat = F.get_features(universe)

    tic = tICA()
    tic.fit([feat])
    tic_proj = tic.transform([feat])[0]
